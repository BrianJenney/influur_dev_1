...
import { Clock, CheckCircle, AlertCircle, Eye, ThumbsUp, MessageCircle, Share2, ExternalLink } from "lucide-react"
...
const ACTIVE_CAMPAIGNS = [
  {
    ...
    status: "in-progress",
    influencers: [
      { id: "1", name: "Alex Johnson", handle: "@alexjdancer", status: "accepted", deliverable: null },
      { id: "2", name: "Maria Garcia", handle: "@mariamusic", status: "pending", deliverable: null },
      { id: "3", name: "Jamal Williams", handle: "@jamalcreates", status: "declined", deliverable: null },
    ],
...
export default function CampaignTracking({ onComplete, campaignData }: CampaignTrackingProps) {
          ...
          <>
              ...
              <Card>
                <CardContent className="p-4 flex flex-col items-center justify-center">
                  ...
                  <p className="text-2xl font-bold">
                    {formatNumber(campaign.influencers.reduce((sum, inf) => sum + (inf.deliverable?.views || 0), 0))}
                  </p>
                </CardContent>
              </Card>
                    ...
                    {formatNumber(
                      campaign.influencers.reduce(
                        (sum, inf) =>
                          sum +
                          (inf.deliverable
                            ? inf.deliverable.likes + inf.deliverable.comments + inf.deliverable.shares
                            : 0),
                        0,
                      ),
                    )}
            ...
            {campaign.status === "completed" && (
              <Tabs defaultValue="performance">
                ...
                <TabsContent value="influencers" className="space-y-4 pt-4">
                        ...
                        <CardContent className="p-4">
                          <div className="flex justify-between items-start">
                            <div className="flex items-center gap-3">
                              <Avatar>
                                <AvatarImage src={`/placeholder.svg?height=40&width=40`} alt={influencer.name} />
                            ...
                            <div className="mt-4 space-y-3">
                              <div className="flex justify-between text-sm">
                                <div className="flex items-center gap-1">
                                  ...
                                  <span>{formatNumber(influencer.deliverable.views)} views</span>
                                </div>
                                <div className="flex items-center gap-1">
                                  <ThumbsUp className="h-4 w-4 text-muted-foreground" />
                                  <span>{formatNumber(influencer.deliverable.likes)} likes</span>
                                </div>
                                <div className="flex items-center gap-1">
                                  <MessageCircle className="h-4 w-4 text-muted-foreground" />
                                  <span>{formatNumber(influencer.deliverable.comments)} comments</span>
                                </div>
                                <div className="flex items-center gap-1">
                                  <Share2 className="h-4 w-4 text-muted-foreground" />
                                  <span>{formatNumber(influencer.deliverable.shares)}</span>
                                  <span> shares</span>
                                </div>
                              </div>
                              ...
                              <Button variant="outline" size="sm" className="w-full">
                                ...
                                View Post on {influencer.deliverable.platform}
                              </Button>
                            </div>
                          )}
                        </CardContent>
                ...
                <TabsContent value="platforms" className="space-y-4 pt-4">
                  <Card>
                    ...
                    <CardContent className="p-0">
                      ...
                      >
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={PLATFORM_DATA} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis dataKey="name" />
                            <YAxis />
                            <ChartTooltip content={<ChartTooltipContent />} />
                            <Legend />
                            <Bar dataKey="value" fill="var(--color-value)" name="Views" />
                          </BarChart>
                        </ResponsiveContainer>
                      </ChartContainer>
                    </CardContent>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-medium">Proposals Sent</p>
                      <p className="text-sm text-muted-foreground">June 15, 2023</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-medium">Waiting for Content Creation</p>
                      <p className="text-sm text-muted-foreground">1 of 3 influencers accepted</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-medium text-muted-foreground">Content Review</p>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-medium text-muted-foreground">Campaign Completion</p>
                      <p className="text-sm text-muted-foreground">Expected by July 15, 2023</p>
                    </div>
                  </div>
                </div>
                ...
                <Card>
                  <CardContent className="p-4">
                    <h3 className="font-medium mb-2">Influencer Status</h3>
                        ...
                        <div key={influencer.id} className="flex justify-between items-center">
                          <div className="flex items-center gap-2">
                            <Avatar className="h-8 w-8">
                              <AvatarImage src={`/placeholder.svg?height=32&width=32`} alt={influencer.name} />
                              <AvatarFallback>{influencer.name.substring(0, 2)}</AvatarFallback>
                            </Avatar>
